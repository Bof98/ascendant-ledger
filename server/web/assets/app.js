import { formatAmount, utcDateRange } from './presentation.js?v=20260910-audit2';
import { renderOperations, disposeOperations } from './operations.js?v=20260910-audit2';
...
async function route(){
  ...
  try{await handler();}catch(e){if(e.name==='AbortError')return;content.innerHTML=`<div class="callout bad"><strong>Unable to load this page.</strong><br>${escapeHtml(e.message)}</div>`;}
}
...
async function renderDashboard(){
  ...
  content.innerHTML=`
    <div class="callout">CSV ledger · ${realmLabel()} · ${d.coverage?.income || d.coverage?.cashflow || d.coverage?.balance ? 'Imported statement periods in the selected UTC date range.' : 'No statements imported in this date range. Missing amounts are shown as —.'} Live captures are shown separately in <a href="#/live-operations">Live overview</a>.</div>
...
const statementDefs={
  income:{title:'Income Statement',sections:[['Revenue',[['Sales','sales']]],['Cost of Goods Sold',[['COGS','cogs'],['Freight Out','freight_out']]],['Gross Profit',[['Gross Profit','gross_profit','subtotal']]],['Operating & Other',[['Construction','construction'],['Exchange Fees','exchange_fees'],['Salaries','salaries'],['Training','training'],['Poaching','poaching'],['Accounting Overhead','accounting_overhead'],['Bond Interest Expense','bond_interest_expense'],['Bond Interest Income','bond_interest_income'],['Donations','donations'],['Achievements / Referrals / PA','achievements_referrals_pa'],['Patent Conversion','patent_conversion'],['Bond Defaults','bond_defaults'],['Bond Writeoffs','bond_writeoffs']]],['Results',[['Net Income','net_income','total'],['Core Business Result (Derived)','core_business_result','total'],['Other Comprehensive Income','other_comprehensive_income']]]]},
...
async function renderStatement(type){
  setLoading(`Loading ${statementDefs[type].title}…`); const d=await api(`/api/statements/${type}${dateQuery()}`); const rows=d.rows||[]; if(!rows.length){content.innerHTML='<div class="empty">No imported statements in the selected UTC date range.</div>';return;}
  let selected=0;
