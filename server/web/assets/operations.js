...
export async function renderOperations(ctx) {
  ...
  const cash = value => value == null ? '—' : esc(money(value));
  ...
  function overview(snapshot, service, live) {
    ...
    const buildingCards = buildings.map(b => {
      ...
      return `<article class="ops-building"><div class="ops-building-head"><div><h3>${esc(b.name || b.raw_name || 'Building')}</h3><p>Level ${esc(b.level ?? '—')} · ${esc(b.product || b.likely_product || 'No product recorded')}</p></div>${pill(b.is_idle ? 'Idle' : b.status || 'Active', b.is_idle ? 'warn' : 'good')}</div>${progress === null ? '' : `<progress max="100" value="${progress}" aria-label="${esc(b.name || 'Building')} progress">${Math.round(progress)}%</progress>`}<div class="ops-building-meta"><span>${esc(b.countdown || (b.is_idle ? 'Ready for production' : 'Timing unavailable'))}</span><span>${cash(b.profit_hr)}/hr estimated</span></div><div class="ops-building-meta"><span>Batch ${number(b.active_qty)}</span><span>Current stock ${number(b.current_stock)}</span></div></article>`;
    }).join('');
    ...
    const statements = ['income_statement', 'balance_sheet', 'cashflow_statement'].map(key => {
      ...
      return card(title, table(['Line', 'Captured value'], list(statement.rows).map(row => [esc(row.k ?? ''), esc(row.text ?? '—')]), 'No statement captured yet.'), statement.period || statement.as_of || 'Latest captured statement; separate from CSV ledger');
    }).join('');
    return `<div class="ops-freshness">${freshness}</div>${snapshot.captured_at && !snapshot.live ? warning('These figures are from an older capture. Check the original console for capture status before acting.') : ''}<div class="grid kpi-grid ops-kpis">${metric('Cash', cash(snapshot.cash), 'Latest captured balance')}${metric('Company value', cash(snapshot.company?.value), 'Last company snapshot')}${metric('Estimated profit / hr', cash(snapshot.profit_hr), 'Building estimates; not realized income')}${metric('Warehouse at market', cash(snapshot.warehouse?.totals?.value_at_market), 'Current captured inventory')}${metric('Idle buildings', snapshot.captured_at ? number(snapshot.idle_buildings) : '—', `${buildings.length} buildings captured`)}${metric('Company level', number(snapshot.level), snapshot.xp_pct == null ? 'Progress unavailable' : `${snapshot.xp_pct}% to next level`)}</div>${health}<div class="ops-columns">${card('Buildings', `<div class="ops-buildings">${buildingCards || '<div class="ops-empty">No buildings captured yet.</div>'}</div>`)}<div class="ops-stack">${card('Warehouse', table(['Product', 'Quantity', 'At cost', 'At market'], warehouses.map(row => [esc(row.name), number(row.quantity), cash(row.value_at_cost), cash(row.value_at_market)])))}${card('Pending exchange sales', table(['Product', 'Quantity', 'Price', 'Value'], list(snapshot.pending_sales).map(row => [esc(row.name), number(row.qty), cash(row.price), cash(row.value)])))}</div></div><details class="ops-statements"><summary>Latest captured financial statements</summary><p>Captured statements show the latest game view. Import CSV exports for the ledger’s historical reports and reconciliation.</p><div class="ops-stack">${statements}</div></details>`;
  }
  ...
  function strategy(data) {
    ...
    return `<div class="grid kpi-grid ops-strategy-kpis">${metric('Forecast cash change', cash(forecast.expected_cash_delta), `Over ${forecast.horizon_hours ?? '—'} hours; projected`)}${metric('Observed cash change', cash(actual.cash_delta), actual.status || 'Observation unavailable')}${metric('Expected to date', cash(actual.expected_to_date), 'Projection over observed period')}${metric('Variance', cash(actual.variance), 'Observed less expected')}</div><div class="ops-columns">${card('Recommended actions', `<div class="ops-recommendations">${recs || '<div class="ops-empty">No recommendations available.</div>'}</div>`, 'Use the original console to manage execution.')}${card('Plan outcomes', table(['Plan', 'Created', 'Observed hours', 'Cash change', 'Variance', 'Status'], list(data.accuracy).map(row => [esc(row.plan_id), when(row.created_at), number(row.observed_hours), cash(row.cash_delta), cash(row.variance), pill(row.status || 'Unknown')])))}</div>${card('Strategy report', `<pre class="ops-report">${esc(data.report || 'No report available.')}</pre>`)}`;
  }
  ...
  async function refresh() {
    ...
    try {
      ...
      if (currentRoute === 'live-operations' || currentRoute === 'activity') {
        ...
        html = card('Market prices', `<div class="ops-market-filter"><label for="marketSearch">Find a product</label><input class="input" id="marketSearch" type="search" placeholder="Search captured products"></div><div id="marketRows">${table(['Product', 'Price', 'Captured', 'Source'], rows.map(row => [esc(row.product_name || row.product_id), cash(row.price), when(row.captured_at), esc(row.source || 'Capture')]))}</div>`, 'Latest stored quotes. Check the capture time before using a price.');
      }
